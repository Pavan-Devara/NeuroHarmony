...
import com.neuro.neuroharmony.data.repository.GetCoreBeliefScoreRepository

class GetCoreBeliefScoreViewModel: ViewModel() {

    var loginAPICallStatus = MutableLiveData<ResourceStatus>() // livedata for observing login API call status
    var loginResponseLiveData = MutableLiveData<GetCoreBeliefScoreResponse>() // live data for getting Login response

    /**
     * Signup in API call
     */
    fun getcorebeliefscore() {
        if (NeuroHarmonyApp.getInstance()!!.isConnected!!) {
            loginAPICallStatus.value = ResourceStatus.loading()
            GetCoreBeliefScoreRepository.getcorebelirfscoree() { isSuccess, response ->
