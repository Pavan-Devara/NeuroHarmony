...
object ActiveChatUsersListRepository {

    private val job = Job()
    private val scope = CoroutineScope(Dispatchers.Main + job)

    fun activeuserslistchat(onResult:  (isSuccess: Boolean, response: ListofActiveAvailableUsersChatResponse?) -> Unit) {

        scope.launch {
            NotificationApiService.instance.activechatinterface().enqueue(object :
