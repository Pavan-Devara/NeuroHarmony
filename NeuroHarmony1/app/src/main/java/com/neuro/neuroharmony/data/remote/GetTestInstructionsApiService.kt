...
interface GetTestInstructionsApiService {

    @GET("/test/instruction-set-v1/?")
    fun instructions(
    ...
    @GET("test/baseline-score/?")
    fun baselineresult(
        @Query("partner_key") partner_key: String?,
        @Header("Authorization") token: String = "Token " + PrefsHelper().getPref(
            "Token"
        )
    ): Call<BaselineResultResponse>

    companion object {
        val instance: GetTestInstructionsApiService by lazy {
            ...
            if (BuildConfig.DEBUG) {
                ...
                val requestInterceptor = Interceptor { chain ->
                    val url = chain.request()
                        .url()
                        .newBuilder()
                        .addPathSegment(PrefsHelper().getPref("test_type"))
                        .addPathSegment("/")
                        .build()
            ...
            Retrofit.Builder()
                ...
                .build()
                .create(GetTestInstructionsApiService::class.java)
        }
