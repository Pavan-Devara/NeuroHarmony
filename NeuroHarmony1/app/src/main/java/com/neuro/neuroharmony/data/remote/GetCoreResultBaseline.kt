...
interface GetCoreResultBaseline {

    @GET("test/baseline-score/3")
    fun baselineresultcore(
        @Query("partner_key") partner_key: String?,
        @Header("Authorization") token: String = "Token " + PrefsHelper().getPref(
            "Token"
        )
    ): Call<BaselineCoreResult>

    companion object {
        val instance: GetCoreResultBaseline by lazy {
            ...
            Retrofit.Builder()
                ...
                .build()
                .create(GetCoreResultBaseline::class.java)
        }
