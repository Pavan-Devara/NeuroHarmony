...
object FinalizeSentRequestsRepo {
    private val job = Job()
    private val scope = CoroutineScope(Dispatchers.Main + job)

    fun finalizesentrequestsrepo(onResult:  (isSuccess: Boolean, response: ResponseSentReceivedRequestFinalizeMatch?) -> Unit) {

        scope.launch {
            NotificationApiService.instance.finalizesentusers().enqueue(object :
