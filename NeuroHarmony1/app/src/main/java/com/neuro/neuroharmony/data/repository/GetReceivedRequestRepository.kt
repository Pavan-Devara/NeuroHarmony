...
object GetReceivedRequestRepository {

    private val job = Job()
    private val scope = CoroutineScope(Dispatchers.Main + job)

    fun receivedrequestsrepo(onResult:  (isSuccess: Boolean, response: UsersSentRequestsResponse?) -> Unit) {

        scope.launch {
            NotificationApiService.instance.receivedrequestsinterface().enqueue(object :
