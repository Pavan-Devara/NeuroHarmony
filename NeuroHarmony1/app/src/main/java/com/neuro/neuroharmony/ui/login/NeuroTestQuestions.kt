...
open class NeuroTestQuestions : BaseActivity(), CompoundButton.OnCheckedChangeListener {
...
//    private val updateDialog = object : Runnable {
//        override fun run() {
//            warningToPause()
//            mainHandler.postDelayed(this, 10000)
////            Handler().postDelayed({
////                Toast.makeText(this@NeuroTestQuestions, "Please select", Toast.LENGTH_SHORT).show()
////            }, 10000)
//        }
//    }
    ...
    @SuppressLint("WrongViewCast", "ResourceAsColor", "NewApi")
    ...
    override fun onStart() {
        ...
        progressBar.progress = question_counter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_neuro_test_questions)
        ...
        if (group_question_counter >= 15){
            group_counter = group_counter+1
            group_question_counter = 0
        }
        question_counter = (group_counter*15)+group_question_counter+1
...
//        mainHandler = Handler(Looper.getMainLooper())
    ...
    private fun questionToBeDisplayed(
        ...
        try {
            ...
            for (i in 0 until array_obj_questions_obj_options.length()){
                ...
                when (order_number){
                    1 -> option_A_mild_neuro_test.text = array_obj_questions_obj_options.getJSONObject(i).getString("option_label")
                    2 -> option_A_strong_neuro_test.text = array_obj_questions_obj_options.getJSONObject(i).getString("option_label")
                    3 -> option_B_mild_neuro_test.text = array_obj_questions_obj_options.getJSONObject(i).getString("option_label")
                    4 -> option_B_strong_neuro_test.text = array_obj_questions_obj_options.getJSONObject(i).getString("option_label")
                }
            }
    ...
    override fun onCheckedChanged(buttonView: CompoundButton?, isChecked: Boolean) {
        ...
        var rb1 = findViewById<RadioButton>(R.id.option_A_mild_neuro_test)
        var rb2 = findViewById<RadioButton>(R.id.option_A_strong_neuro_test)
        var rb3 = findViewById<RadioButton>(R.id.option_B_mild_neuro_test)
        var rb4 = findViewById<RadioButton>(R.id.option_B_strong_neuro_test)

        if (isChecked) {
            if (buttonView!!.id == R.id.option_A_mild_neuro_test) {
                rb2.isChecked = false
                option_B_mild_neuro_test.setTextColor(Color.WHITE)
                option_A_strong_neuro_test.setTextColor(Color.WHITE)
                option_B_strong_neuro_test.setTextColor(Color.WHITE)
                rb3.isChecked = false
                rb4.isChecked = false
                option_A_mild_neuro_test.setTextColor(getColor(R.color.background))
            }
            if (buttonView.id == R.id.option_A_strong_neuro_test) {
                rb1.isChecked = false
                rb3.isChecked = false
                rb4.isChecked = false
                option_B_strong_neuro_test.setTextColor(Color.WHITE)
                option_B_mild_neuro_test.setTextColor(Color.WHITE)
                option_A_mild_neuro_test.setTextColor(Color.WHITE)
                option_A_strong_neuro_test.setTextColor(getColor(R.color.background))
            }
            if (buttonView.id == R.id.option_B_mild_neuro_test) {
                rb2.isChecked = false
                rb1.isChecked = false
                rb4.isChecked = false
                option_A_mild_neuro_test.setTextColor(Color.WHITE)
                option_A_strong_neuro_test.setTextColor(Color.WHITE)
                option_B_strong_neuro_test.setTextColor(Color.WHITE)
                option_B_mild_neuro_test.setTextColor(getColor(R.color.background))
            }

            if (buttonView.id == R.id.option_B_strong_neuro_test) {
                rb2.isChecked = false
                rb1.isChecked = false
                rb3.isChecked = false
                option_B_mild_neuro_test.setTextColor(Color.WHITE)
                option_A_mild_neuro_test.setTextColor(Color.WHITE)
                option_A_strong_neuro_test.setTextColor(Color.WHITE)
                option_B_strong_neuro_test.setTextColor(getColor(R.color.background))
            }
        }
    ...
    private fun setupListeners(
        ...
        Neuro_questions_continue.setOnClickListener {
            time1=0
            if (rb1.isChecked || rb2.isChecked || rb3.isChecked || rb4.isChecked) {
                ...
                Log.d("group question counter", group_question_counter.toString())
                Log.d("start Time", startTime)
                Log.d("end Time", endTime)
                Log.d("sessionId", sessionId)
                Log.d("testType", testType.toString())
    ...
    fun retest(){
        if (question_counter != totalQuestions) {
            viewModel1reset.resettestvm(NEUROTEST, session_id)
        }
    }
